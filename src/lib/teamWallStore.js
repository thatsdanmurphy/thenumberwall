...
export async function getActiveWallsWithSignals(limit = 5) {
  ...
  const { data: entries } = await supabase
    .from('team_wall_entries')
    .select('wall_id, name, added_by, added_at, entry_type, went_pro')
    .in('wall_id', wallIds)
    .eq('status', 'active')

  const signals = new Map()
  for (const id of wallIds) {
    signals.set(id, { entryCount: 0, legendCount: 0, legendNames: [], proNames: [], allNames: [], contributors: new Set(), lastActivityAt: null })
  }
  for (const e of entries || []) {
    ...
    } else if (e.went_pro && e.name) {
      s.proNames.push(e.name)
    }
    if (e.name) s.allNames.push(e.name)
  ...
  return walls.map(w => {
    ...
    // Top names: legend seeds first, then went-pro crowd, then everyone else
    const topNames = [...new Set([...s.legendNames, ...s.proNames, ...s.allNames])]
    return {
      ...w,
