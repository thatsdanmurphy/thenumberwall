...
const NUMBER_DATA = {
  ...
  10: { tier:TIER.LEGEND, players:[
    ...
    { name:"Pelé",             sport:"Soccer",era:"1956–1977",team:"Santos",                stat:"1281", statLabel:"Career Goals (all comps)",  role:"Forward",      icon:"⚽" },
  ...
  15: { tier:TIER.LEGEND, players:[
    { name:"Nikola Jokić",     sport:"NBA", era:"2015–pres", team:"Nuggets",                stat:"3",     statLabel:"MVP Awards",               role:"Center",       icon:"🏀" },
...
function tierLabel(tier) {
  if (tier === TIER.SACRED)    return "⬡ SACRED — RETIRED LEAGUE-WIDE";
...
export default function WornNumbers() {
  ...
  return (
    <div style={{
      minHeight:"100vh", background:"#080c10", color:"white",
      ...
      <style>{`
        ...
        @media (min-width:480px) {
          .grid-wrap { grid-template-columns:repeat(10,1fr); gap:5px; }
          .cell-label { font-size:13px; }
          .panel-number { font-size:64px; }
        }
        ...
        @media (min-width:960px) {
          .grid-wrap { grid-template-columns:repeat(13,1fr); gap:6px; }
      ...
      {/* ── HEADER ── */}
      <div style={{
        borderBottom:"1px solid rgba(255,255,255,0.08)", padding:"12px 14px 10px",
        background:"rgba(0,0,0,0.5)", backdropFilter:"blur(10px)",
        position:"sticky", top:0, zIndex:100,
      }}>
        {/* Wordmark — centered */}
        <div style={{textAlign:"center",marginBottom:10}}>
          <div className="mono subtext" style={{color:"rgba(255,255,255,0.3)",marginBottom:2}}>
            LEGENDS LIVE HERE
          </div>
          <h1 className="bebas wordmark" style={{margin:0,lineHeight:1}}>
            THE NUMBER WALL
          </h1>
        </div>

        {/* Tier key — centered, sits under wordmark */}
        ...
        {/* Sport filter — centered scrollable row */}
      ...
      <div style={{padding:"12px 10px",maxWidth:1100,margin:"0 auto"}}>
        ...
        {/* ── HEATMAP GRID ── */}
        <div className="grid-wrap" style={{marginBottom: selected !== null ? 16 : 0}}>
          {Array.from({length:99},(_,i) => {
            const num    = i + 1;
            const tier   = NUMBER_DATA[num]?.tier || TIER.UNWRITTEN;
            const allPlayers = NUMBER_DATA[num]?.players || [];
            const hasPlayers = filteredPlayers(num).length > 0;
            const dimmed = sport !== "ALL" && !hasPlayers;

            // Player count for weight — use filtered count when sport filter active
            const filteredCount = filteredPlayers(num).length;
            const playerCount = sport === "ALL" ? allPlayers.length : filteredCount;
            const isSelected = selected===num;

            // Sacred only glows Sacred when filter is ALL or matches the sport that retired it
            ...
            // When filtered out, treat cell as dim/unwritten visually
            let colors;
            if (dimmed && !isSelected) {
              colors = { ...TIER_COLORS.UNWRITTEN, glow:"none" };
            } else {
              const baseColors = tierColors(effectiveTier, isSelected);
              // Weight: Legend cells glow harder with more players
              colors = baseColors;
              if (effectiveTier === TIER.LEGEND && !isSelected) {
                if (playerCount >= 3) {
                  colors = {
                    ...baseColors,
                    bg:     "rgba(210,60,0,0.6)",
                    text:   "rgba(255,185,100,1)",
                    border: "rgba(255,100,20,0.85)",
                    glow:   "0 0 16px rgba(255,100,20,0.65), 0 0 32px rgba(255,80,0,0.3)",
                  };
                } else if (playerCount === 2) {
                  colors = {
                    ...baseColors,
                    bg:     "rgba(205,55,0,0.52)",
                    text:   "rgba(255,165,80,1)",
                    border: "rgba(255,90,10,0.7)",
                    glow:   "0 0 14px rgba(255,90,10,0.52), 0 0 28px rgba(255,80,0,0.2)",
                  };
                }
              }
            }
              ...
              <div key={num}
                ...
                style={{
                  ...
                  boxShadow: selected===num
                    ? `0 0 0 2px ${colors.accent}, ${colors.glow}`
                    : colors.glow,
                  color: colors.text,
        ...
        {/* ── SELECTED PANEL ── */}
        {selected !== null && (
          <div className="panel-in panel-wrap" style={{
            background:"rgba(255,255,255,0.03)",
            border:`1px solid ${selectedColors.border}`,
            borderRadius:16,
            boxShadow:`0 0 40px ${selectedColors.border}22`,
          }}>
            {/* Panel header */}
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:16,gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:16}}>
                <div className="bebas panel-number" style={{
                  lineHeight:1, letterSpacing:2,
                  color: selectedColors.text,
                  textShadow:`0 0 30px ${selectedColors.border}`,
                }}>
                  #{selected}
                </div>
                <div>
                  ...
                  {selectedData?.tier === TIER.SACRED && (
                    <div style={{
                      ...
                      background:"rgba(200,220,255,0.1)", border:"1px solid rgba(200,220,255,0.35)",
                      borderRadius:6, padding:"2px 8px", fontSize:10,
                      color:"#C8DCFF", fontFamily:"'Share Tech Mono',monospace", letterSpacing:2,
                    }}>SACRED</div>
                  )}
                  {selectedData?.tier === TIER.RISING && (
                    <div style={{
                      display:"inline-block", marginTop:4,
                      background:"rgba(120,220,20,0.1)", border:"1px solid rgba(120,220,20,0.3)",
                      borderRadius:6, padding:"2px 8px", fontSize:10,
                      color:"#8FD920", fontFamily:"'Share Tech Mono',monospace", letterSpacing:2,
                    }}>ACTIVE</div>
                  )}
                </div>
              </div>
            ...
            {/* ── WRITTEN CARDS (Sacred numbers) ── */}
            {selectedContent ? (
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {selectedContent.intro && (
                  <div className="mono" style={{fontSize:11,letterSpacing:2,color:"rgba(200,220,255,0.55)",marginBottom:4}}>
                    {selectedContent.intro}
                  </div>
                )}
                {selectedContent.stacks.map((stack, i) => (
                  <div key={i} className="written-card" style={{animationDelay:`${i*80}ms`}}>
                    {stack.connector && (
                      <div style={{
                        borderLeft:`3px solid ${selectedAccent}`, paddingLeft:12, marginBottom:14,
                        fontSize:13, color:"rgba(255,255,255,0.5)", fontStyle:"italic", lineHeight:1.6,
                      }}>
                        {stack.connector}
                      </div>
                    )}
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                      <span className="bebas" style={{fontSize:20,letterSpacing:2,color:"#fff"}}>{stack.name}</span>
                      <span style={{
                    ...
                    {/* Body */}
                    <p style={{fontSize:14,lineHeight:1.65,color:"rgba(255,255,255,0.65)",margin:"0 0 12px"}}>
                      {stack.body}
                    </p>
                    {/* Wait What */}
                    {stack.waitWhat && (
                      <div style={{borderTop:"1px solid rgba(255,255,255,0.08)",paddingTop:12}}>
                        <div className="mono" style={{fontSize:10,letterSpacing:3,color:selectedAccent,marginBottom:6}}>
                          WAIT, WHAT?
                        </div>
                        <p style={{
                          fontSize:13, lineHeight:1.65, fontStyle:"italic",
                          color:"rgba(255,255,255,0.5)", margin:0,
                          borderLeft:`2px solid ${selectedAccent}`, paddingLeft:12,
                        }}>
                          {stack.waitWhat}
                        </p>
                      </div>
                ...
                {/* Sacred banner */}
                {selectedContent.sacredLine && (
                  <div style={{
                    marginTop:4, padding:"12px 16px", borderRadius:8,
                    background:"rgba(200,220,255,0.06)", border:"1px solid rgba(200,220,255,0.2)",
                    textAlign:"center",
                  }}>
                    <span className="mono" style={{fontSize:11,letterSpacing:2,color:"#C8DCFF"}}>
                      {selectedContent.sacredLine}
                    </span>
                  </div>
                )}
            ...
            ) : selectedData?.tier === TIER.UNWRITTEN ? (
              /* ── UNWRITTEN ── */
              <div style={{padding:"24px 0 16px"}}>
                ...
                <div style={{borderTop:"1px solid rgba(255,255,255,0.07)",paddingTop:20,textAlign:"center"}}>
                  <div className="mono" style={{fontSize:10,letterSpacing:3,color:"rgba(255,255,255,0.2)",marginBottom:10}}>
                    THINK WE MISSED SOMEONE?
                  </div>
                  <button
                    style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:8,padding:"9px 20px",fontFamily:"'Share Tech Mono',monospace",fontSize:12,letterSpacing:2,color:"rgba(255,255,255,0.4)",cursor:"pointer",transition:"all 0.2s"}}
                    onMouseEnter={e=>{e.currentTarget.style.background="rgba(255,255,255,0.1)";e.currentTarget.style.color="rgba(255,255,255,0.7)";e.currentTarget.style.borderColor="rgba(255,255,255,0.3)";}}
                    onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,0.05)";e.currentTarget.style.color="rgba(255,255,255,0.4)";e.currentTarget.style.borderColor="rgba(255,255,255,0.15)";}}
                    onClick={()=>alert("Community nominations coming soon — this is where you will be able to nominate a legend for #"+selected+".")}>
                    NOMINATE A LEGEND →
                  </button>
                </div>
              ...
              /* ── PLAYER CARDS (Legend / Rising) ── */
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {selectedPlayers.length > 0 ? selectedPlayers.map((p, i) => (
                  <div key={i} className="player-card" style={{animationDelay:`${i*80}ms`}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                      <div style={{display:"flex",alignItems:"center",gap:12}}>
                        <span style={{fontSize:24}}>{p.icon}</span>
                        <div>
                          <div className="bebas" style={{fontSize:20,letterSpacing:2,lineHeight:1,marginBottom:2}}>
                            {p.name}
                          </div>
                      ...
                      {p.stat !== "–" && (
                        <div style={{textAlign:"right"}}>
                          <div className="stat-num" style={{
                            color:selectedColors.text,
                            textShadow:`0 0 12px ${selectedColors.border}`,
                          }}>
                            {p.stat}
                          </div>
                          <div style={{fontSize:9,color:"rgba(255,255,255,0.35)",fontFamily:"'Share Tech Mono',monospace",letterSpacing:1,maxWidth:140,textAlign:"right",lineHeight:1.3}}>
                            {p.statLabel}
                          </div>
                        </div>
                      )}
        ...
        {selected === null && (
          <div style={{
            ...
            <span style={{color:"rgba(255,255,255,0.35)"}}>PICK A NUMBER.</span>
          </div>
        )}
